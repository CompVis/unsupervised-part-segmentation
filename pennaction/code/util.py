# SHAMELESSLY COPIED FROM https://github.com/tensorflow/tensorflow/blob/r1.8/tensorflow/python/ops/distributions/util.py
...
def assert_close(x, y, data=None, summarize=None, message=None, name="assert_close"):
    ...
    with ops.name_scope(name, values=[x]):
        ...
        if not x.dtype.is_integer:
            assertions += [
                assert_integer_form(
                    x,
                    message="'{}' cannot contain fractional components.".format(
                        x.op.name
                    ),
                )
            ]
  ...
  """
    with ops.name_scope(name, values=(cond, true_vector, false_vector)):
        ...
        if cond.dtype != dtypes.bool:
            raise TypeError(
                "%s.dtype=%s which is not %s" % (cond.name, cond.dtype, dtypes.bool)
            )
