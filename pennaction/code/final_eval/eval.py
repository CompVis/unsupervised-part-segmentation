...
class MatrixHook(Hook):
    ...
    def before_step(self, step, fetches, feeds, batch):
        for k in [self.generated_key, self.vis0_key, self.vis1_key]:
            if k in self.model.outputs.keys():
                fetches[k] = self.model.outputs[k]
            elif k in self.model.inputs.keys():
                pass
            else:
                raise ValueError("key {} not found".format(k))
    ...
    def after_step(self, step, results):
        for k in [self.generated_key, self.vis0_key, self.vis1_key]:
            if k in self.model.inputs.keys():
                pass
            else:
    ...
    def after_epoch(self, epoch):
        ...
        make_matrix(
            ...
            rowvis_key=self.vis0_key,
            colvis_key=self.vis1_key,
