...
import warnings
warnings.warn("module moved to src.nn", DeprecationWarning)
...
def mumford_shah(x, alpha, lambda_):
    ...
    smoothness_cost = tf.where((alpha * g) < lambda_, r, tf.zeros_like(g, dtype= g.dtype))
    contour_cost = tf.where((alpha * g) >= lambda_, r, tf.zeros_like(g, dtype=g.dtype))
...
def colorize(value, vmin=0, vmax=1, cmap="viridis"):
    ...
    """
    # shamelessy copied from https://gist.github.com/jimfleming/c1adfdb0f526465c99409cc143dea97b
    # TODO: code this into edflow
    # TODO: make this work on batches
    ...
    # gather
    cm = mpl.cm.get_cmap(cmap if cmap is not None else 'gray')
...
def unpool_features_gathered(feature_vectors, mask):
    """
    feature_vectors : [B, parts, features]
    mask : [B, h, w]
    output : [b, h, w, n_parts, features]
    """
    bs, h, w = mask.shape.as_list()
    ...
    output, _ = tf.map_fn(
        lambda x: (tf.gather(x[0], x[1], axis=0), x[0]),
        (feature_vectors, mask),
        dtype=(tf.float32, tf.float32),
    )
    return output


def augm(t, contrast_var=0.05, brightness_var=0.05, saturation_var=0.05, hue_var=0.05):
    """t in range [0, 1]"""
...
# def augment_img_at_mask(img, mask, augm_function=augm):
#     """mask
...
#     --------
#         img = data.astronat()
#         mask = np.zeros((2, 512, 512, 10), dtype=np.float32)
#         mask[:, 100:412, 100:412, :] = 1.0

#         a = augment_img_at_mask(tf.to_float(imgt) / 255, mask)
...
#     """
#     blending_map = mask
#     augmented = augm_function(img, contrast_var=0.3, saturation_var=0.3, hue_var=0.3)
#     return tf.expand_dims(img, -2) * (1 - tf.expand_dims(blending_map, -1)) + tf.expand_dims(blending_map, axis=-1) * tf.expand_dims(augmented, axis=-2)
...
def augment_img_at_mask(img, mask, augm_function=augm):
    ...
    """
    blending_map = mask
    augmented = tf.stack([augm_function(img, contrast_var=0.3, saturation_var=0.3, hue_var=0.3) for i in range(mask.shape.as_list()[-1])], axis=3)
