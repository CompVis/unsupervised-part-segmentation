...
def make_figure_1(data: dict, root: str, config: dict, global_step: int):
    ...
    remapped_sampled_inferred = denseposelib.remap_parts(
        np.argmax(sampled_segmentation, -1) + 1, best_remapping
    )
    remapped_sampled_inferred = remapped_sampled_inferred.astype(np.int32)
    ...
    for i in range(
        ...
        # ipdb.set_trace()
        new_sampled_inferred = remapped_sampled_inferred[i]
        ...
        image_container.append(colors[new_sampled_inferred])
        ...
        for image, name in zip(
            ...
            write_slice(
                (i, imageutils.convert_range(image, [0, 1], [-1, 1])),
...
def main(target_dir: str, config_path: str, single: bool, n_processes: int):
    ...
    python eval/eval_iclr_01/eval_01.py ../logs/2019-11-12T16-34-07_SB_model_ablation/eval/2019-11-13T10-58-16_hard_no_entropy/infer/020000/ eval/eval_iclr_01/deepfashion.yaml --single --n-processes 1
