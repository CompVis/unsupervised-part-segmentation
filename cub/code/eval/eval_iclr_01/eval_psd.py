...
def make_figure_1(data: dict, root: str, config: dict, global_step: int):
    ...
    dp_new_part_list = ["background", "full Torso", "arm", "left leg", "right leg"]
    ...
    remapped_gt_segmentation = groundtruth_segmentation
    remapped_gt_segmentation[remapped_gt_segmentation == dp_new_part_list.index("right leg")] = dp_new_part_list.index("left leg") # remap right leg to left leg
    ...
    colors = make_mask_colors(len(set(dp_new_part_list)), background_id=0)
    ...
    for i in range(
        ...
        old_inferred_colors = make_mask_colors(n_inferred_parts, background_id=0)
...
def make_mask_colors(
    ...
    if n_parts > 10:
        ...
        colors = cc.cm.glasbey_light(np.linspace(0, 1, n_parts), alpha=False, bytes=False)[:, :3]
    import seaborn as sns
    # colors = sns.color_palette("colorblind", n_parts)
